...
import { createDistributionAction } from "@/lib/actions/distributions";
...
export function DistributionForm({
  ...
  async function handleSubmit(
    ...
    if (!studentId || !deviceSerial || !handoverDate) {
  ...
  return (
    <FormWithLetterhead
      ...
      description="Record a new device handover to a student"
    >
      <form action={formAction} className="space-y-6">
        ...
        <div>
          <label htmlFor="handoverDate" className="block text-sm font-medium text-slate-700">
            Handover Date
          </label>
        ...
        {isSuccess && (
          <div className="rounded-lg border border-green-300 bg-green-50 p-4">
            <p className="text-sm text-green-800">
              Distribution {state.id} created successfully
            </p>
          </div>
        )}
